...
class ControllerDecenNode(Node):
    '''
    This class is used to create a node for the Decentralized MPC controller for the robot.
    The node will subscribe to the robot and load pose and publish the control input to the robot.
    '''
    def __init__(self,model=None,controller=None,init=None,simulation_time=None,CONTROLLER_ENABLE = False,PWM_ENABLE=True,agent = 1):
        '''
        ...
        self.i_dt = 0      
        self.agent = agent                                                         # counter for logging data
        ...
        self.x_warm1 = np.vstack((self.x_warm[0:6,:],self.x_warm[12:18,:]))
        self.x_warm2 = np.vstack((self.x_warm[6:12,:],self.x_warm[12:18,:]))
        self.u_warm1 = self.u_warm[0:4,:]
        self.u_warm2 = self.u_warm[4:8,:]
        SLEEP = True
        if SLEEP:
            self.sleep_time = 5
        else:
        ...
        self.xref1 = np.vstack([self.xref[0:6],self.xref[12:18]])
        self.xref2 = np.vstack([self.xref[6:12],self.xref[12:18]])                              # get first reference point from the array
    ...
    def update_reference_point(self):
        ...
        position_trigger_error = 0.03 #0.01
        angle_trigger_error = 0.05        
        ...
        if np.linalg.norm(self.err_x_Log[:,[self.i_dt]]) <= position_trigger_error and abs(self.err_the_Log[:,[self.i_dt]]) <= angle_trigger_error:      # 0.03 0.02 quaternion     # 0.002 0.01
            ...
            if self.sleep_time_count >= self.sleep_time:
                ...
                if self.point == self.xref_array.shape[1]-1:
                    ...
                    self.xref = self.xref_array[:,[self.point]]    
                    self.xref1 = np.vstack([self.xref[0:6],self.xref[12:18]])
                    self.xref2 = np.vstack([self.xref[6:12],self.xref[12:18]])                                        
    ...
    def solve_mpc(self):
        ...
        if self.agent == 1:
            ...
            x_Log1 = np.vstack([self.x_Log[0:6,self.i_dt],self.x_Log[12:18,self.i_dt]]).reshape((-1,1)) 
            self.u_Log[0:4,[self.i_dt]],x_temp1,u_temp1 = self.controller.solve_mpc(x_Log1,self.xref1,self.x_warm1,self.u_warm1)
            t_stop = time.perf_counter()
        elif self.agent == 2:
            t_start = time.perf_counter() 
            x_Log2 = np.vstack([self.x_Log[6:12,self.i_dt],self.x_Log[12:18,self.i_dt]]).reshape((-1,1)) 
            self.u_Log[4:8,[self.i_dt]],x_temp2,u_temp2 = self.controller.solve_mpc(x_Log2,self.xref2,self.x_warm2,self.u_warm2)
            t_stop = time.perf_counter()
        ...
        if self.agent == 1:
            self.x_warm1 = np.hstack((x_temp1[:,1:],np.reshape(x_temp1[:,-1],(-1,1))))
            self.u_warm1 = u_temp1[:,0:]
        elif self.agent == 2:
            self.x_warm2 = np.hstack((x_temp2[:,1:],np.reshape(x_temp2[:,-1],(-1,1))))
            self.u_warm2 = u_temp2[:,0:]
        ...
        if self.agent == 1:
            self.cost_Log[:,[self.i_dt]] = self.controller.running_cost(x_Log1,self.xref1,self.controller.Q,self.u_Log[0:4,[self.i_dt]],self.controller.R)
        elif self.agent == 2:
            self.cost_Log[:,[self.i_dt]] = self.controller.running_cost(x_Log2,self.xref2,self.controller.Q,self.u_Log[4:8,[self.i_dt]],self.controller.R)
        ...
        if self.agent == 1:
            ...
            # print("Error rotation: "+str(self.err_the_Log[:,[self.i_dt]][0][0]))
            pass
